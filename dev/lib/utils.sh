...
function package_7z() {
  ...
  nicely "7z a -t7z -mmt=$(nproc) -bso0 -bsp1 '${output_basename}.7z' ."
  popd > /dev/null
}
export -f package_7z
...
function package_zip() {
  ...
  mkdir -p "$(dirname "${output_basename}")"
  pushd "${input_dir}" > /dev/null
  nicely "7z a -tzip -mmt=$(nproc) -bso0 -bsp1 '${output_basename}.zip' ."
  popd > /dev/null
}
export -f package_zip
...
function package_all() {
  ...
  parallel ::: \
    ...
    "package_gz '${input_dir}' '${output_basename}'" \
    "package_zst '${input_dir}' '${output_basename}'" \
    "package_7z '${input_dir}' '${output_basename}'" \
    "package_zip '${input_dir}' '${output_basename}'"
}
export -f package_all
